...
function zxt2(){   
    ...
    //定义咱们的svg画布空间容器
    let svg = d3.select('#zhexian2')
             .append('svg')
             .attr('width',width)
             .attr('height',height);

     //创建线性比例尺，使用坐标轴必备
    const yScale = d3.scaleLinear().domain([160, 0]).range([0, width - margin*2]);
    //const yScale = d3.scaleLinear().domain([1, 0]).range([width/2 - margin*2, width ]);
